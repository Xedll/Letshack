...
export const TeamCard = ({ Title, description, marks, contacts }) => {
				...
				<div className='flex gap-x-2'>
					{marks &&
						marks.map((element, index) => {
							return <Tag key={index} text={element.title} color={"ourPink"} />
						})}
				</div>
				<div className='font-semibold text-ourPink text-base'>{Title}</div>
			</div>

			<div className={` flex-col gap-y-3 mb-4  ${isMouseOn ? "flex" : "hidden"}`}>
				<p className='text-ourPink text-base font-semibold'>Описание</p>
				<p className='text-[#1D1F2480] font-xs '>{description}</p>
			</div>
			<button className='font-sm text-[#FFF] bg-ourPink border-rounded rounded-3xl w-full h-fit py-4 '>{contacts}</button>
		</div>
	)
}
